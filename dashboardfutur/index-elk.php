...
// --- RÉCUPÉRATION DES DONNÉES (Source Elasticsearch) ---

$elkUrl = 'http://mon-serveur-elk:9200/server-inventory/_search';
$username = 'elastic'; // Si besoin
$password = 'changeme'; // Si besoin

// La requête DSL Elastic pour tout récupérer (ou filtrer directement ici)
$query = [
    "size" => 1000, // Attention à la pagination sur les gros parcs !
    "query" => [
        "match_all" => (object)[] // Astuce pour forcer un objet vide JSON {}
    ]
];

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, $elkUrl);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
curl_setopt($ch, CURLOPT_USERPWD, "$username:$password"); // Auth Basic
...
if ($httpCode === 200 && $result) {
    ...
    // Transformation des données ELK vers notre format interne
    // ELK renvoie souvent dans ['hits']['hits'][...]['_source']
    foreach ($data['hits']['hits'] as $hit) {
        $source = $hit['_source'];
        
        // Mapping : On sécurise avec '??' si un champ manque dans ELK
        $servers[] = [
            'hostname' => $source['host']['name'] ?? 'Inconnu',
            'ip'       => $source['ip_address'] ?? '0.0.0.0',
            'env'      => $source['tags']['environment'] ?? 'Production', // Souvent dans les tags
            'filebeat' => isset($source['service']['filebeat']) && $source['service']['filebeat'] === 'running',
            'zabbix'   => isset($source['agent']['zabbix']),
            'xlr'      => isset($source['custom']['xlr_installed'])
        ];
    }
} else {
    // Mode dégradé ou affichage d'erreur
    echo '<div class="alert alert-warning m-4">Connexion ELK échouée (Code: '.$httpCode.')</div>';
