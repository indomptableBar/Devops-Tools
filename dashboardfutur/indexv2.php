...
//- RÉCUPÉRATION DES DONNÉES (Source JSON) ---

// URL de ton API ou chemin vers un fichier généré par Ansible/Puppet
// Exemple : 'http://mon-api-interne/api/servers' ou './data/servers.json'
$sourceUrl = 'servers.json'; 
...
try {
    ...
    if ($jsonContent === false) {
        throw new Exception("Impossible d'accéder à la source de données.");
    }

    // On décode le JSON en tableau PHP associatif
    $servers = json_decode($jsonContent, true);

    if ($servers === null) {
        throw new Exception("Erreur de décodage JSON. Vérifiez le format.");
    }

} catch (Exception $e) {
    // En cas d'erreur, on affiche une alerte Bootstrap et on arrête tout proprement
    echo '<div class="alert alert-danger m-4">Erreur critique : ' . $e->getMessage() . '</div>';
    $servers = []; // Tableau vide pour éviter que le reste de la page ne plante
}
